...
from zeromq_channel.channel import ZeromqChannel, MessageDataHeader
...
import json
from flatbuffer_serializer.serializers import fb_create_rawimage, fb_create_pointcloud
import time
...
if __name__ == "__main__":
    foxglove_channel = ZeromqChannel()
    color_channel = {
        "topic": "colorImage",
        "encoding": "flatbuffer",
    ...
    pointcloud_channel = {
        ...
        "id": 1,
        "schema": base64.b64encode(get_schema("PointCloud")).decode("ascii")
    }

    foxglove_channel.advertise(color_channel, pointcloud_channel)

    foxglove_channel.poll(2000, handle_studio_message)
    ...
    with dai.Device(pipeline) as device:
        ...
        xyz = create_xyz(resolution[0], resolution[1],
                         np.array(M_right).reshape(3, 3))
        ...
        # main stream loop
        while True:
            foxglove_channel.poll(1, handle_studio_message)
            if COLOR and qRgb.has():
                raw_img = fb_create_rawimage({"sec": 1}, "yuv420", resolution[0], resolution[1], resolution[0] * 3, qRgb.get().getData())
                if 0 in channel_to_subscription:
                    full_message = MessageDataHeader.pack(
                                        1, channel_to_subscription[0], time.time_ns()) + raw_img
                    # foxglove_channel.send(full_message)
                # cv2.imshow("color", qRgb.get().getCvFrame())

            pcl_data = np.array(queue.get().getFirstLayerFp16()).reshape(-1)
            
            # pcl_data = np.array(queue.get().getFirstLayerFp16())
            pc = fb_create_pointcloud({"sec": 1}, pcl_data)
            if 1 in channel_to_subscription:
                full_message = MessageDataHeader.pack(
                                    1, channel_to_subscription[1], time.time_ns()) + pc
                foxglove_channel.send(full_message)
